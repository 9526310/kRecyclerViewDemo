...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private ArrayList<Student> list,searchList,tempList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //初始化一些虚拟数据
        tempList = new ArrayList<Student>();//临时保存数据的集合，查询时会用到
        list = new ArrayList<Student>();//数据列表集合
        list.add(new Student("张三",25,"",false,R.mipmap.hjr));
        list.add(new Student("李四",23,"http://img2.imgtn.bdimg.com/it/u=1396521707,1973816781&fm=26&gp=0.jpg",false,R.mipmap.huli));
        list.add(new Student("王五",20,"http://img2.imgtn.bdimg.com/it/u=1031599332,2025124400&fm=26&gp=0.jpg",false,R.mipmap.nazha));
    ...
    private void initRecyclerView(int btnId)
    {
        ...
        adapter.setItemClickListener(new MyRecyclerviewAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position) {
                selectedStudent = list.get(position);
                //选中状态置为相反的
                selectedStudent.setSelected(!selectedStudent.isSelected());
        ...
        //设置选中个数监听
        adapter.setCountCheckBoxListener(new MyRecyclerviewAdapter.CountCheckBoxListener() {
            @Override
    ...
    public void onClick(View view) {
        switch (view.getId())
        {
            ...
            case R.id.btn_add:
                //数据添加一个假数据
                list.add(new Student("新加"+(++addIndex),25,"",false,getHeadImageId(addIndex)));
                //刷新列表
    ...
    {
        final Student selStudent = list.get(position);
